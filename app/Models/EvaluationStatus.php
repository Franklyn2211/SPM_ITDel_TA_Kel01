...
{
    protected $table = 'ref_evaluation_status';
    ...
    public function selfEvaluationForms()
    {
        return $this->hasMany(SelfEvaluationForm::class, 'status_id', 'id');
    }

    public function selfEvaluationDetails()
    {
        return $this->hasMany(SelfEvaluationDetail::class, 'status_id', 'id');
    }

    public static function generateNextId(): string
    {
        $maxNum = (int) self::where('id', 'like', 'ES%')
            ->selectRaw("MAX(CAST(SUBSTRING(id, 3) AS UNSIGNED)) as maxnum")
            ->value('maxnum');

        $next = 'ES' . str_pad((string) ($maxNum + 1), 3, '0', STR_PAD_LEFT);
